...
export default function Archive({auth, ...props}){
    ...
    const order_status = [
        {name : 'Dibatalkan', style : 'bg-danger-1 text-danger-1 '}, 
        {name : 'Menunggu pesanan diambil', style : 'bg-gray-500 text-gray-500 '}, 
        {name : 'Pesanan sedang diambil', style : 'bg-orange-500 text-orange-500 '}, 
        {name : 'Pesanan telah diambil', style : 'bg-yellow-500 text-yellow-500'},
        {name : 'Menunggu proses pembayaran', style : 'bg-emerald-500 text-emerald-500'},
        {name : ['Pesanan telah dibuat oleh admin', 'Pesanan telah dibayar'], style : 'bg-blue-500 text-blue-500'},
        {name : 'Pesanan sedang dikerjakan', style : 'bg-purple-500 text-purple-500'},
        {name : 'Pesanan selesai dikerjakan', style : 'bg-pink-500 text-pink-500'},
        {name : 'Pesanan sedang diantar', style : 'bg-rose-500 text-rose-500'}, 
        {name : 'Selesai', style : 'bg-success-1 text-success-1 '},
    ]

    const updateStatus = (e, id, order_id) => {
        e.preventDefault()
        Swal.fire({
            text: "Anda yakin mengambil ID Pesanan "  + order_id + " ?",
            icon: "question",
            reverseButtons: true,
            ...
            if (result.isConfirmed) {
                router.visit('/pesanan/'+id+'/status', { method : 'post' })
            }
        });
    }
    ...
    return(
        <AdminLayout user={auth.user} >
            <h1 className="p-5 text-xl border-b border-light-3">Arsip Pesanan</h1>
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="table-header">
                                ...
                                <th className="py-3 px-2">Status Pesanan</th>
                            ...
                            : orders.map((order, index) => {
                                return(
                                    <tr key={index} className={'text-black ' + ( index  % 2 == 0 ? 'bg-light-1 ' : 'bg-white ')}>
                                        ...
                                        <td className='py-3 px-2 text-center'><span className={'py-1 px-2 text-sm font-semibold rounded-md bg-opacity-25 ' + order_status[order.order_status].style}>{order_status[order.order_status].name}</span></td>        
