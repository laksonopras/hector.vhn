...
export default function Pickup({auth, ...props}){
    ...
    const updateStatus = (e, id, order_id) => {
        e.preventDefault()
        Swal.fire({
            text: "Apakah Anda sudah mengambil ID Pesanan "  + order_id + " ?",
            icon: "question",
            reverseButtons: true,
            ...
            if (result.isConfirmed) {
                router.visit('/pesanan/status', { method : 'post' , data: {id: id, status: 3}})
            }
        });
    }
    ...
    return(
        <AdminLayout user={auth.user} >
            <h1 className="p-5 text-xl border-b border-light-3">Pesanan Sedang Diambil</h1>
