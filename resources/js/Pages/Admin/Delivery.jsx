...
export default function Delivery({auth, ...props}){
    ...
    const updateStatus = (e, id, order_id) => {
        e.preventDefault()
        Swal.fire({
            text: "Apakah ID Pesanan "  + order_id + " telah selesai diantar?",
            icon: "question",
            reverseButtons: true,
            ...
            if (result.isConfirmed) {
                router.visit('/pesanan/status', { method : 'post' , data: {id: id, status: 9}})
            }
        });
    }
    ...
    return(
        <AdminLayout user={auth.user} >
            <h1 className="p-5 text-xl border-b border-light-3">Pesanan Sedang Diantar</h1>
                                    ...
                                    <tr key={index} className={'text-black ' + ( index  % 2 == 0 ? 'bg-light-1 ' : 'bg-white ')}>
                                        ...
                                        <td className='px-1 text-center'>
                                            <div className="flex justify-center space-x-2">
                                                ...
                                                <a onClick={(e) => updateStatus(e, order.id, order.receipt)} className="p-1 btn-warning"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" ><path fill="currentColor" d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75zM20.71 7.04a.996.996 0 0 0 0-1.41l-2.34-2.34a.996.996 0 0 0-1.41 0l-1.83 1.83l3.75 3.75z"/></svg></a>
